...
$macro x_out_sparse1 (ord(m)<=ord(n) and N_out(t,k,m)>0)
...
*x_out.fx(t,k,n,m)$(ord(m)<17 and N_out(t,k,n)>0 and ord(m)>=ord(n))=0;

* Scale the starting number of EVs.
*n_start(n)=round(0.89*n_start(n));
...
* Constraints
c(S_clust1(t,n))..                       n_bat(t-1,n) + N_start(n)$(ord(t)=1) - sum((k,m)$x_out_sparse1,x_out(t,k,m,n)) + x_in(t,n)$x_in_sparse1 =e= sum(m$k_cond(n,m),n_aux(t,n,m));

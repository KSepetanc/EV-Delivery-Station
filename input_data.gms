$eolCom //
set tl 15 min time periods /t1*t672/;
set t(tl) 15 min time periods /t1*t96/;      //Time period can be increased up to t672, but increase the N_start so the model is feasable. (96)
...
$macro hours(x) (mod(ord(t),4)=x)

parameter N_start(n),N_out(tl,k,n),Eta(n,m),Lambda_week(w,t),K_ch(n),K_dis(n),N_clus,IM_Lambda(t);
* Load data from GDX. GDX is created using xlsx_to_gdx.gms from excel.
$gdxin data.gdx
$load N_start,N_out,Eta,Lambda_week,IM_Lambda
$gdxin
...
Lambda(t)$hours(1)=sum(w,Lambda_week(w,t))/card(w);

parameter Gamma /8/;        //Uncertanty budget
parameter Delta_lambda(t);  //Price change interval, €/MWh
Delta_lambda(t)$hours(1)=smax(w,abs(Lambda_week(w,t)-Lambda(t)));
...
N_out(t,k,n)=N_out(t,k-3,n-(N_min-1));   //Change k-3 to k to make EVs come earlier (perfect information)
*N_out(t,k,n)=N_out(t,k,n+1);            //Uncomment to make EVs come with gihger SoE (perfect information)
